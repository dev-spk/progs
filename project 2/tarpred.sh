...
#UTRBase="$Home/TargetMiner/hg18_3utr.txt" # old;update; if target not in file, TMiner writes out NOTHING !!! 
...
#	    $(zcat /dev/null >$ResultFile.gz) # empty essential; some error
	    ...
	    echo -e "The prediction is:\n$(cat $ResultFile)\n"
	    score3=$(sed '1,1d' $ResultFile | awk '{if($3 != ""){printf $3}}') # TT_parser here 
	    if [[ $score3 == "" ]] # seen an empty file
	    then
		score3="NA"
	    fi
	    $(cat /dev/null >$ResultFile) # empty essential
	    ...
	    record="$miRID\t$refseqID\t$score2\t$score3\t$score4\t$score5\t$score6\t$score7\t$FamID\t$miRSeq\t$seed\t$tarSeq"
	    ...
	    $(rm -f "$UTR1 $UTR1_TM $experivalid1 $OutFile") # donot delete the results file until all done
done; } <$UTRs

rm -f "$miR1 $miR1_TM $miRmature1 $miRseed1"

done; } <$miRs

$(rm -f "$ResultFile $ResultFile.gz") # all done
