...
print $CON "MIReNA\tMiPred\tTripSVM\tProMiR\tmiRPara\tLabel\n";
...
chdir "$Home";
=pod
# Tool 7 (miRBoost: would need a training set of known/potential miRNAs; test and train sets in regular fasta format)
#-- calculate 187 features for input sequences
 # ---- -L 400: upper bound for all sequence sizes in input fasta. It can be appropriately reduced to reduce running time.
  #---- -D -1|1: class label for all sequences. It is required to treat positive and negative samples separately, then join them into a unique training or testing file.
print "Beginning to run the miRBoost classifier .......................\n";
chdir("$Home/miRBoost");
print "Run gFold on test datasets....\n";
!system("gFoldmulti/gFold -s $intext -L 400 -DATA 1 > test_187.txt") or die "\n";
#print "Run gFold on the train datasets...\n";
#!system("gFoldmulti/gFold -s train.fasta -L 400 -DATA -1 > train_187.txt") or die "\n";
  
#  -- extract selected features from all features
my $select = 'human_selected_features.txt'; # if human
#---- $SELECT = 'whole_selected_features.txt';# if cross-species

$|=1; # Set $AUTOFLUSH: flush all files opened for output before forking the child process so as to avoid creating duplicate output
print "Beginning to extract features from the test selected....\n";
!system("./src/extractfeature.pl -s $select -i test_187.txt -o test_selected.txt") or die "\n";
#print "Beginning to extract features from the train selected ...\n";
#!system("./src/extractfeature.pl -s $select -i train_187.txt -o train_selected.txt") or die "\n";

#-- run miRBoost
#---- be default, delta = 0.35
$|=1; # Set $AUTOFLUSH: flush all files opened for output before forking the child process so as to avoid creating duplicate output
print "run miRBoost ...\n";
`pwd`;
!system "$Home/miRBoostweb/miRBoost", "-i", "$Home/miRBoostweb/test_selected.txt", "-t", "$Home/miRBoostweb/train_selected.txt", "-d", ".35" or die "\n";# output written to results.txt by default
=cut
...
=pod
open (my $OUX5, "$Home/miRBoost/results.txt") or die "\n";
$rec = <$OUX5>;
chomp($rec);
$line .= "$rec";
#print "The current line after miRboost is \n $line \n";
close $OUX5 if $OUX5 != 0;
system "rm", "$Home/miRBoost/results.txt";
=cut
