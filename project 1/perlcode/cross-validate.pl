...
foreach $input (@inputs) {
    ...
    $foldcount = int($count / 5 + .5); # round the size of each subset to 1/10-th size
    $foldnum = 0;
    ...
    for($i=0; $i<5; $i++) { # write-out to files; index for VALID
	for($j=0; $j<5; $j++) { # index for TEST
		if($j == $i) {next}
		print "Fold $i\n";
        	$basename = "$input-$i-$j"; # indx. VALID - indx. TEST
        	...
        	open(VALID, "+>$basename.valid") or
                	die "Couldn't open $basename.valid\n";
        	print VALID $fold[$i];
		print TEST $fold[$j]; 
		for($k=0; $k<5; $k++) { # index for TRAIN
    			if($k == $i || $k == $j) {next} # index of TRAIN ne to index of TEST or the index of VALID
...
# Run targets	
# Concatenate $i-$j positive subset with the corresponding $i-$j negative subset, use $basename-$i-$j
for($i=0; $i<5; $i++) {
     for($j=0; $j<5; $j++) {  
	if($j == $i) {next} # next iteration
	$basename1 = "$inputs[0]-$i-$j"; 
	$basename2 = "$inputs[1]-$i-$j";
	$basename3 = "$inputs[0]"."$inputs[1]-$i-$j"; # positive, negative samples
	$cmdline = "cat $basename1.data $basename2.data >$basename3.data";
        system($cmdline);
	$cmdline = "cat $basename1.test $basename2.test >$basename3.test";
	system($cmdline);
        $cmdline = "cat $basename1.valid $basename2.valid >$basename3.valid";
        system($cmdline);
	unlink("$basename1.data", "$basename1.test", "$basename1.valid", "$basename2.data", "$basename2.test", "$basename2.valid");
